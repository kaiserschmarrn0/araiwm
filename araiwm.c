...
arai_kill(int arg)
{
    ...
    if (xcb_icccm_get_wm_protocols_reply(connection,
	    xcb_icccm_get_wm_protocols_unchecked(connection, temp, ewmh->WM_PROTOCOLS),
	    &protocols,
	    NULL))
...
main(int argc, char **argv)
{
    ...
    if (argc > 2) {
    	...
    	printf("araiwm: path to config: %s\n", argv[1]);
        char scan[STR_MAX];
        uint32_t val;
        FILE *fconf;
        if (fconf = fopen(argv[1], "r")) {
            while (!feof(fconf)) {
                fscanf(fconf, "%s ", &scan);
                if (strcmp(scan, "unfocuscol") == 0 || strcmp(scan, "focuscol") == 0)
                    fscanf(fconf, "= %x", &val);
                else fscanf(fconf, "= %d", &val);
    	        printf("araiwm: %s = %d\n", scan, val);
    	        for (int i = 0; i < sizeof(items)/sizeof(*items); i++)
                    if (strcmp(scan, items[i].key) == 0) *items[i].var = val;
            }
        fclose(fconf);
        } else printf("araiwm: failed to open %s\n", argv[1]);
